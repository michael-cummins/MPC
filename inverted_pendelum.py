...
#always set to false, gekko uses a remote server in the US by default and its really bad

#defining how long the simulation will run
m.time = np.linspace(0,8,100) 
end_loc = int(100*6.2/8.0)

#Paramters
m1a = m.Param(value = 10) #weight of the cart
m2a = m.Param(value = 1) #weight of mass at the top of the cart
...
ua = m.Var(value = 0) #initial value
#Initial State Varaibles
theta_a = m.Var(value = 0)
qa = m.Var(value = 0)
ya = m.Var(value = -1)
va = m.Var(value = 0)

epsilon = m.Intermediate(m2a/(m2a + m1a))

# Defining the rate at which our state changes
m.Equation(ya.dt() == va)
m.Equation(va.dt() == ua - epsilon*theta_a)
